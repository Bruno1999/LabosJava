...
public class Glavna {
    ...
    public static void main(String[] args) {
        ...
        //ucitavanje predmeta
        try(BufferedReader bufReaderPredmet = new BufferedReader(new FileReader(new File("dat/predmet.txt")));
        ){

            List<String> datotekaPredmet = bufReaderPredmet.lines().collect(Collectors.toList());

            for (int i = 0;i < datotekaPredmet.size()/BROJ_ZAPISA_DAT_STUDENTI;i++){
                String idPredmet = datotekaPredmet.get(i * BROJ_ZAPISA_DAT_STUDENTI);
                String sifraPredmet = datotekaPredmet.get(i * BROJ_ZAPISA_DAT_STUDENTI+1);
                String nazivPredmet = datotekaPredmet.get(i * BROJ_ZAPISA_DAT_STUDENTI+2);
                String brojEcts = datotekaPredmet.get(i * BROJ_ZAPISA_DAT_STUDENTI+3);
                String nositeljString = datotekaPredmet.get(i * BROJ_ZAPISA_DAT_STUDENTI+4);
                String studentiString = datotekaPredmet.get(i * BROJ_ZAPISA_DAT_STUDENTI+5);
